...
public class opensslkey {
 ...
 const  String pemp8encfooter   = "-----END ENCRYPTED PRIVATE KEY-----" ;
    const String pemcerheader = "-----BEGIN CERTIFICATE-----";
    const String pemcerfooter = "-----END CERTIFICATE-----";
...
// static byte[] pkcs8encprivatekey;
    ...
    public RSACryptoServiceProvider obtieneLlavePrivadaRSA(String pFileName, String keyPassword) 
    {
        RSACryptoServiceProvider rsaParams = null;
        //if (args.Length == 1)
        //    if (args[0].ToUpper() == "V")
                verbose = true;
        ...
        if (filename == "")
        {
            ultimoMensaje += " [obtieneLlavePrivadaRSA] Verifique la configuración del certificado. La ruta está vacía.";
            return rsaParams;
        }
        if (!File.Exists(filename))
        {
            Console.WriteLine("File \"{0}\" does not exist!\n", filename);
            ultimoMensaje += " [obtieneLlavePrivadaRSA] Verifique la configuración del certificado. El certificado no existe: " + filename;
            return rsaParams;
        }
        ...
        if (pemstr.StartsWith("-----BEGIN"))
            ...
            rsaParams = DecodeDERKey(filename, keyPassword);
...
// ------- Decode DER public, private or pkcs8 key ----------------
  public RSACryptoServiceProvider DecodeDERKey(String filename, String keyPassword)
 {
	RSACryptoServiceProvider rsa = null ;
	byte[] keyblob = GetFileBytes(filename);
    if (keyblob == null)
    {
        ultimoMensaje += " [DecodeDERKey] Verifique el nombre de archivo del certificado. No se puede encontrar el archivo:"+filename;
        return rsa;
    }
	...
	rsa =  DecodeEncryptedPrivateKeyInfo(keyblob, keyPassword);	//PKCS #8 encrypted
    if (rsa != null)
    {
    ...
    ultimoMensaje += " [DecodeDERKey] Verifique el certificado. No es una llave pública válida de formato binario DER.";
	return rsa;
}
...
//------- Parses binary asn.1 EncryptedPrivateKeyInfo; returns RSACryptoServiceProvider ---
    public static RSACryptoServiceProvider DecodeEncryptedPrivateKeyInfo(byte[] encpkcs8, String filePassword)
 {
 // encoded OID sequence for  PKCS #1 rsaEncryption szOID_RSA_RSA = "1.2.840.113549.1.1.1"
 // this byte[] includes the sequence byte and terminal encoded null 
...
SecureString secpswd = obtieneClaveSegura( filePassword);
...
if(Win32.CertStrToName(X509_ASN_ENCODING, DN, CERT_X500_NAME_STR, IntPtr.Zero, null, ref cbName, IntPtr.Zero))
 ...
 private static SecureString obtieneClaveSegura(String keyPassword)
    {
     SecureString password = new SecureString();
     foreach (char c in keyPassword)
     {
 ...
 public string HexString2Ascii(string hexString)
    {
        ...
        {
            ultimoMensaje += " [HexString2Ascii] Error al obtener el número de certificado en base64.";
            return "";
        }
    }

 public X509Certificate2 certificadox509(string fileName) 
    {
        //Test for correct number of arguments.
        if (fileName.Length < 1)
        {
            ultimoMensaje += " [certificadox509] La ruta del certificado está vacía. Verifique la configuración de certificados.";
        ...
        try
        {
            ...
            //Console.WriteLine("{0}Serial Number hex: {1}{0}", Environment.NewLine, x509.SerialNumber);
            //Console.WriteLine("{0}Serial Number ascii: {1}{0}", Environment.NewLine, HexString2Ascii(x509.SerialNumber));
        ...
        {
            ultimoMensaje += " [certificadox509] Verifique la configuración de certificados. La ruta del certificado no pudo ser encontrada: " + fileName;
            Console.WriteLine("Error: The directory specified could not be found.");
            return null;
        }
        catch (IOException)
        {
            ultimoMensaje += " [certificadox509] Verifique que el certificado existe. No se pudo acceder al certificado: " + fileName;
            Console.WriteLine("Error: A file in the directory could not be accessed.");
            return null;
        }
        catch (NullReferenceException)
        {
            ultimoMensaje += " [certificadox509] Verifique que el certificado tenga extensión .cer. No se tiene acceso al archivo: " + fileName;
            Console.WriteLine("File must be a .cer file. Program does not have access to that type of file.");
            return null;
        }
    }

 public string leeCertificadoPEM(string fileName)
    {
        //Test for correct number of arguments.
        if (fileName.Length < 1)
        {
            ultimoMensaje += " [leeCertificadoPEM] Verifique la configuración de certificados. La ruta del certificado PEM está vacía.";
            return null;
        }
        try
        {
            ...
            sr.Close();
            if (pemstr.StartsWith(pemcerheader) && pemstr.EndsWith(pemcerfooter))
            {
            ...
            {
                ultimoMensaje += " [leeCertificadoPEM] Verifique el certificado con extensión PEM. El formato es inconsistente: "+fileName;
            }
            return pemstr;
        }
        ...
        {
            ultimoMensaje += " [leeCertificadoPEM] Verifique la configuración de certificados. La ruta del certificado no pudo ser encontrada: " + fileName;
            Console.WriteLine("Error: The directory specified could not be found.");
            return null;
        }
        catch (IOException)
        {
            ultimoMensaje += " [leeCertificadoPEM] Verifique que el certificado existe. No se pudo acceder al certificado: " + fileName;
            Console.WriteLine("Error: A file in the directory could not be accessed.");
            return null;
        }
        catch (NullReferenceException)
        {
            ultimoMensaje += " [leeCertificadoPEM] Verifique que el certificado tenga extensión .pem. No se tiene acceso al archivo: " + fileName;
